...
function NavbarXX(props) {

  const classesnav = useStyles();
  ...
  const signIn=(event)=>{
    ...
    setOpenSignIn(false)//on sign in close the modal
  }
...
<Modal //modal is used as a html which occurs on an event like a popup from material Ui with click out
        ...
        onClose={()=>resetField()}
      >
        <div style={modalStyle} className={classesnav.paper}>
...
{user?(
<div className="customNav"><Row verticalAlignment="center" className="rowcol" size={6}>
<Column >
  <h2>shortyUrl</h2>
    </Column>
<Column >
    ...
    <Column className ="col-sm-auto" size={4}>
    <div><div>
      <span>Sort By Group:</span>
    <select value={grp} onChange={(e)=>grpUpdate(e.target.value)}>
...
<Column >
    ...
    <Column  >
    <div><button className="btn btn-warning" onClick={()=>signOut() }>Sign Out</button></div>
</Column>
</Row></div>):
(
    ...
    <div className="customNav">
       ...
       <Row verticalAlignment="center" className="rowcol" size={6}>
    ...
    <Column >
        <div><button className="btn btn-success" onClick={()=>setOpenSignIn(true)}>SignIn </button></div>
        
        </Column>
        <Column  >
        <div><button className="btn btn-warning" onClick={()=>setOpenSignUp(true)}>SignUp </button></div>
    </Column>
    </Row>
